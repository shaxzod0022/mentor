...
export default function AddCourseModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.image) {
      setError('Kurs rasmini tanlash shart!');
      return;
    }
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || 'Kurs yaratishda xatolik yuz berdi');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative bg-white w-full max-w-xl rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-8 pb-4 flex items-center justify-between border-b border-slate-50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-900">Yangi kurs qo'shish</h2>
              <p className="text-slate-400 text-xs font-medium">Kurs ma'lumotlarini kiriting</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="text-xs text-slate-500">
                <p className="font-bold text-slate-700 mb-1">Rasm yuklash</p>
                <p>Tavsiya etilgan o'lcham: 800x600px. Maksimal hajm: 2MB.</p>
                <button 
                  type="button"

...
export default function GradingModal({ isOpen, onClose, submission, onSuccess }) {
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <div>
                ...
                <p className="text-xs text-slate-400 font-medium">{submission.student?.firstName} {submission.student?.lastName}</p>
             </div>
          </div>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-4 bg-slate-50 p-6 rounded-3xl border border-slate-100">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-bold text-slate-900">{submission.material?.name}</span>
            </div>
            <div className="flex items-center justify-between">
          ...
          <div className="space-y-2">
            <label className="text-sm font-bold text-slate-500 ml-1">Baho (max 10)</label>
            <input 
              type="number"
              step="0.1"
              max="10"
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={20} /> : 'Baholashni saqlash'}
          </button>
        </form>
      </div>
    </div>

    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm">Siz ishtirok etayotgan barcha o'quv kurslari</p>
        </div>
      ...
      {filteredCourses.length === 0 ? (
        <div className="p-20 bg-white rounded-4xl border border-dashed border-slate-200 text-center space-y-4">
          ...
          <div className="max-w-xs mx-auto">
            ...
            <p className="text-sm text-slate-500">Siz hozircha hech qanday kursga biriktirilmagansiz yoki qidiruv natijasi yo'q.</p>
          </div>
        </div>
      ) : view === 'grid' ? (
          ...
          {filteredCourses.map((course) => (
            ...
            >
              <div className="h-48 overflow-hidden relative">
                <img 
                  src={course.image ? `http://localhost:8080${course.image}` : 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&q=80'} 
                  alt={course.name}
                ...
                <div className="absolute inset-0 bg-linear-to-t from-slate-900/60 to-transparent flex items-end p-6">
                  <div className="w-full flex items-center justify-between text-white">
                     <span className="text-xs font-bold uppercase tracking-widest bg-indigo-500/50 backdrop-blur-md px-3 py-1 rounded-full">Kurs</span>
              ...
              <div className="p-6">
                <h3 className="text-xl font-bold text-slate-900 mb-2 group-hover:text-indigo-600 transition-colors">{course.name}</h3>
                <p className="text-sm text-slate-500 line-clamp-2 mb-6 leading-relaxed">
                  {course.description || "Ushbu kurs haqida hozircha qo'shimcha ma'lumot yo'q."}
                </p>
                <div className="flex items-center justify-between">
                  <div className="flex -space-x-3">
                    ...
                    {(course.students?.length || 0) === 0 && (
                      <p className="text-xs font-bold text-slate-400">Hozircha o'quvchilar yo'q</p>
                    )}
                  </div>
          ...
          {filteredCourses.map((course) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-bold text-slate-900 group-hover:text-indigo-600 transition-colors truncate">{course.name}</h3>
                <p className="text-sm text-slate-500 line-clamp-1 truncate max-w-md">{course.description}</p>
              </div>
              <div className="hidden md:flex items-center gap-12 px-8">
                  <div className="text-center">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Davomiylik</p>
                      <p className="text-sm font-bold text-slate-700">12 Haftalik</p>
                  </div>
                  <div className="text-center">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">O'quvchilar</p>
                      <p className="text-sm font-bold text-slate-700">{course.students?.length || 0} ta o'quvchi</p>
                  </div>
              </div>

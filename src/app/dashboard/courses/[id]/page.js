...
export default function StudentCourseDetails() {
  ...
  if (loading) return <div className="flex items-center justify-center h-96 text-slate-400">Yuklanmoqda...</div>;
  if (!course) return <div className="text-center p-20">Kurs topilmadi</div>;

  const getMaterialStatus = (materialId) => {
    const sub = submissions.find(s => s.material?._id === materialId || s.material === materialId);
    if (!sub) return { label: 'Topshirilmagan', color: 'slate' };
    if (sub.status === 'graded') return { label: `Baholangan: ${sub.grade}/10`, color: 'emerald', icon: CheckCircle };
    if (sub.status === 'reviewed') return { label: 'Ko\'rib chiqilgan', color: 'indigo', icon: ShieldCheck };
    return { label: 'Topshirilgan', color: 'amber', icon: Clock };
  };

  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <Link href="/dashboard/courses" className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 transition-colors group w-fit">
        <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" />
        <span className="font-bold text-sm uppercase tracking-widest">Kurslarga qaytish</span>
      </Link>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-8 rounded-4xl border border-slate-100 shadow-sm">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                 <h1 className="text-2xl font-black text-slate-900">{course.name}</h1>
                 <p className="text-sm text-slate-500">{materials.length} ta o'quv materiali</p>
              </div>
            </div>

            <div className="space-y-4">
              {materials.length === 0 ? (
                 <div className="text-center py-12 text-slate-400">Hozircha materiallar yuklanmagan</div>
                  ...
                  >
                    <div className="flex items-start gap-4 flex-1">
                      <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-slate-400 group-hover:text-indigo-600 shadow-sm transition-colors mt-1">
                         {material.pdfUrl ? <FileText size={20} /> : <Video size={20} />}
                      </div>
                      <div className="min-w-0">
                        <h4 className="font-bold text-slate-900 mb-1">{material.name}</h4>
                        <p className="text-xs text-slate-500 line-clamp-1 mb-2">{material.description}</p>
                    ...
                    <div className="mt-4 lg:mt-0 flex items-center gap-4 border-t lg:border-t-0 pt-4 lg:pt-0 border-slate-100">
                       <div className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-xs font-bold leading-none
                          ${status.color === 'emerald' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                            status.color === 'indigo' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 
                            status.color === 'amber' ? 'bg-amber-50 text-amber-600 border-amber-100' : 
          ...
          <div className="bg-white p-6 rounded-4xl border border-slate-100 shadow-sm">
             ...
             <div className="space-y-4">
                {course.teachers?.map((t) => (
                   <div key={t._id} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 transition-colors">
                      <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center font-bold text-slate-700">
                         {t.firstName[0]}
                      </div>
                      <div className="min-w-0">
                         <p className="text-sm font-bold text-slate-900 truncate">{t.firstName} {t.lastName}</p>
                         <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Ustoz</p>
                      </div>
                   </div>
                ))}
             </div>
          ...
          <div className="bg-white p-6 rounded-4xl border border-slate-100 shadow-sm">
             ...
             <div className="space-y-4">
                {course.mentors?.map((m) => (
                   <div key={m._id} className="flex items-center gap-3 p-3 rounded-2xl hover:bg-slate-50 transition-colors">
                      <div className="w-10 h-10 bg-emerald-50 text-emerald-600 rounded-xl flex items-center justify-center font-bold">
                         {m.firstName[0]}
                      </div>
                      <div className="min-w-0">
                         <p className="text-sm font-bold text-slate-900 truncate">{m.firstName} {m.lastName}</p>
